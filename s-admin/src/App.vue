<template>
  <v-app>
    ...
    <ConnectionModal :is-open="showConnectionModal" :initial-server-url="serverUrl" :initial-ws-only="wsOnly"
      :initial-path="path" :initial-namespace="namespace" :initial-parser="parser" :is-connecting="isConnecting"
      :error="connectionError" @submit="onSubmit" />
  </v-app>
</template>
  ...
  methods: {
    tryConnect(serverUrl, namespace, auth, wsOnly, path, parser) {
      ...
      const socket = io(serverUrl + namespace, {
        forceNew: true,
        reconnection: false,
        // Needed for cookie-based sticky-sessions
